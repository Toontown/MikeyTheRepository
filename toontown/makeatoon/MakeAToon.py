...
class MakeAToon:
    ...
    def __createAvatar(self, tf, hood=0, trackChoice=[-1, -1]):
        ...
        base.cr.csm.sendCreateAvatar(self.toon.style, self.names[0], self.slot, tf, hood, trackChoice)
    ...
    def enterTypedName(self, *args):
        ...
        if message:
            ...
            self.acceptOnce('BadNameAcknowledge',self.__handleBadNameAcknowledge)
        else:
            self.names[0] = name
            self.finalize()
    ...
    def showNameShopGUI(self):
        ...
        for np in self.pickANameGUIElements:
            np.show()
        if not config.GetBool('want-type-a-name', False):
