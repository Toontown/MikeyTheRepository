...
class DistributedSuitAI(DistributedSuitBaseAI.DistributedSuitBaseAI):
    ...
    def requestBattle(self, x, y, z, h, p, r):
        ...
        if self.pathState == 3:
            ...
            return
        elif self.legType != 2: # SuitLeg.TWalk:
            if self.notify.getDebug():
    ...
    def __beginLegType(self, legType):
        ...
        if legType == 2: # SuitLeg.TWalkFromStreet
            if self.zoneId // 1000 == 7:
                if self.buildingDestination:
                    # funny farm hack bc of fucked bldgs
                    # take over right now
                    self.checkBuildingState()
                    self.openToonDoor()
                    taskMgr.doMethodLater(5, DistributedSuitAI.startTakeOver, self.taskName('ff-takeover'), [self])
            else:
                self.checkBuildingState()
        elif legType == 7: #SuitLeg.TToToonBuilding
            self.openToonDoor()
        elif legType == 6: #SuitLeg.TToSuitBuilding
            self.openSuitDoor()
        elif legType == 9: #SuitLeg.TToCogHQ
            self.openCogHQDoor(1)
        elif legType == 8: #SuitLeg.TFromCogHQ
    ...
    def removeCogFromDoor(self, task):
        self.b_setPathState(2)
        self.stopPathNow()
        name = self.taskName('flyAwayNow')
        taskMgr.remove(name)
    ...
    def openCogHQDoor(self, enter):
        ...
        if enter:
            ...
            taskMgr.doMethodLater(3.0, self.removeCogFromDoor, 'Cog-Deletion')
        else:
            return
