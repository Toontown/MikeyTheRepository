...
class ToontownClientRepository(OTPClientRepository.OTPClientRepository):
    ...
    def __init__(self, serverVersion, launcher = None):
        ...
        self.wantEmblems = base.config.GetBool('want-emblems', 1)
    ...
    def __sendGetAvatarDetails(self, avId, pet=0):
        print '__sendGetAvatarDetails %s %s' % (avId, pet)
        if pet:
    ...
    def dumpAllSubShardObjects(self):
        ...
        if isNotLive:
            try:
                ...
                self.notify.debug('dumpAllSubShardObjects')
            else:
                self.notify.debug('dumpAllSubShardObjects: defaultShard is %s' % localAvatar.defaultShard)
            ...
            ignoredClasses = ('MagicWordManager', 'TimeManager', 'DistributedDistrict', 'FriendManager', 'NewsManager',
                              ...
                              'DistributedSuitInvasionManager')
    ...
    def readDCFile(self, dcFileNames = None):
        ...
        if dcFileNames == None:
            try:
                readResult = dcFile.read(dcStream, "_dc__TTH::nri@%x" % id(dcStream))
