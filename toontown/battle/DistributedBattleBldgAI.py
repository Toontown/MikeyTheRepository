...
class DistributedBattleBldgAI(DistributedBattleBaseAI.DistributedBattleBaseAI):
    ...
    def faceOffDone(self):
        ...
        if not self.ignoreFaceOffDone:
            if self.allToonsResponded():
                ...
                self.timer.stop()
                self.timer.startCallback(TIMEOUT_PER_USER, self._DistributedBattleBldgAI__serverFaceOffDone)
    ...
    def enterFaceOff(self):
        ...
        self.timer.startCallback(self.calcToonMoveTime(self.pos, self.elevatorPos) + FACEOFF_TAUNT_T + SERVER_BUFFER_TIME, self._DistributedBattleBldgAI__serverFaceOffDone)
