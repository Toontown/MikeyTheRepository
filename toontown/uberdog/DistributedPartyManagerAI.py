...
#        taskMgr.doMethodLater(180, self.__getIds, 'DistributedPartyManagerAI___getIds')
    ...
    def partyManagerUdStartingUp(self):
        self.notify.debug("uberdog started up")

    def partyManagerUdLost(self):
        self.notify.warning("uberdog has been lost!")
    ...
    def addPartyRequest(self, hostId, startTime, endTime, isPrivate, inviteTheme, activities, decorations, inviteeIds):
        if hostId != simbase.air.getAvatarIdFromSender():
            ...
            return
        self.notify.info('party requested: host %s, start %s, end %s, private %s, invitetheme %s, activities omitted, decor omitted, invitees %s' % (hostId, startTime, endTime, isPrivate, inviteTheme, inviteeIds))
    ...
    def getPartyZone(self, hostId, zoneId, isAvAboutToPlanParty):
        ...
        if isAvAboutToPlanParty:
            ...
            self.notify.debug('Responding to a get party zone when planning, av,party,zone: %s %s %s' % (avId, partyId, zoneId))
            ...
            if hostId not in self.host2PartyId:
                ...
                self.notify.debug('querying for details against hostId %s' % hostId)
                return
