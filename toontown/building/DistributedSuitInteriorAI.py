...
class DistributedSuitInteriorAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def _DistributedSuitInteriorAI__addToon(self, toonId):
        ...
        self.accept(event, self._DistributedSuitInteriorAI__handleUnexpectedExit, extraArgs = [
            toonId])
    ...
    def elevatorDone(self):
        ...
        if toon:
            if self.bldg.track == "x":
                toon.d_setSystemMessage(0, "Loblao: Welcome to my troll! I hope you brought good gags...")
        ...
        if self._DistributedSuitInteriorAI__allToonsResponded() and self.ignoreElevatorDone == 0:
    ...
    def reserveJoinDone(self):
        ...
        self.responses[toonId] += 1
        if self._DistributedSuitInteriorAI__allToonsResponded() and self.ignoreReserveJoinDone == 0:
            self.b_setState('Battle')
    ...
    def enterElevator(self):
        ...
        self.timer.startCallback(BattleBase.ELEVATOR_T + ElevatorData[ELEVATOR_NORMAL]['openTime'] + BattleBase.SERVER_BUFFER_TIME, self._DistributedSuitInteriorAI__serverElevatorDone)
    ...
    def _DistributedSuitInteriorAI__createFloorBattle(self):
        if self.currentFloor == self.topFloor:
            ...
            bossBattle = 0
        self.battle = DistributedBattleBldgAI.DistributedBattleBldgAI(self.air, self.zoneId, self._DistributedSuitInteriorAI__handleRoundDone, self._DistributedSuitInteriorAI__handleBattleDone, bossBattle = bossBattle)
    ...
    def _DistributedSuitInteriorAI__handleRoundDone(self, toonIds, totalHp, deadSuits):
        totalMaxHp = 0
        for suit in self.suits:
    ...
    def _DistributedSuitInteriorAI__handleBattleDone(self, zoneId, toonIds):
        if len(toonIds) == 0:
            taskName = self.taskName('deleteInterior')
            taskMgr.doMethodLater(10, self._DistributedSuitInteriorAI__doDeleteInterior, taskName)
    ...
    def enterReservesJoining(self):
        self._DistributedSuitInteriorAI__resetResponses()
        self.timer.startCallback(ElevatorData[ELEVATOR_NORMAL]['openTime'] + SUIT_HOLD_ELEVATOR_TIME + BattleBase.SERVER_BUFFER_TIME, self._DistributedSuitInteriorAI__serverReserveJoinDone)
