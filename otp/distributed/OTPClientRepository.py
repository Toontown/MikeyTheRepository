...
from toontown.launcher.TTHLauncher import *
...
class OTPClientRepository(ClientRepositoryBase):
    ...
    def __init__(self, serverVersion, launcher = None, playGame = None):
        ...
        self.tthLauncher = TTHLauncher()
        self.tthLauncher.load()
    ...
    def enterLauncher(self, serverList=[]):
        if serverList:
            self.serverList = serverList
        self.accept('TTHLauncherLoginDone', self.__handleTTHLauncherLoginDone)
        self.tthLauncher.enter()
        
    def exitLauncher(self):
        self.ignore('TTHLauncherLoginDone')

    def __handleTTHLauncherLoginDone(self):
 ...
 'backArgs': []}):
    ...
    def __handleFailedToGetConstantsAck(self):
        ...
        if doneStatus == 'ok':
            self.loginFSM.request('launcher')
            messenger.send('connectionRetrying')
        elif doneStatus == 'cancel':
            self.loginFSM.request('shutdown')
        else:
