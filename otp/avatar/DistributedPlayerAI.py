...
@magicWord(category=CATEGORY_ADM, types=[str]) # This needs a better category. 
def smsg(text):
...
@magicWord(category=CATEGORY_ADM, types=[str])
def smsgall(text):
...
# only top admin can grant others access
@magicWord(category=CATEGORY_HEAD, types=[int])
def setAdminAccess(access=200):
